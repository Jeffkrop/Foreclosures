...
a <- read_csv("//Users/jeffkropelnicki/Desktop/GIS5564_Urban_GIS/Project/foreclosures..csv")
...
Join <- homes_built %>% right_join(foreclosure, by = c("FULLADDRESS")) %>% select(FULLADDRESS, USE1_DESC, YEAR_BUILT, sheriffsal_year, FIRSTBIDAM, Shape_Length, Shape_Area) %>% filter(sheriffsal_year > YEAR_BUILT)

Join$sheriffsal_year <- as.integer(Join$sheriffsal_year)

Join <- Join %>% mutate(owned = sheriffsal_year - YEAR_BUILT) 

mean(Join$owned, na.rm = TRUE) #avg number of years a home was built before a foreclosure 28.81 years

homes_1990 <- Join %>% filter(YEAR_BUILT >= 1990)
homes_before <- Join %>% filter(YEAR_BUILT < 1990)

homes_after_1990 <- Join %>% filter(YEAR_BUILT >= 1990) %>% summarise(count = n())
homes_before_1990 <- Join %>% filter(YEAR_BUILT < 1990 & YEAR_BUILT > 1949) %>% summarise(count = n())
...
homes_per_year_after_1950 <- homes_per_year %>% filter(YEAR_BUILT >= 1950)
mean(homes_per_year_after_1950$count) # Avg number of homes built after 1950 337.6061
