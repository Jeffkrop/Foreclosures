...
Investigate data 
```{r}
more_then_one_foreclosure <- foreclosures_work %>% group_by(FULLADDRESS, sheriffsal_year) %>% summarise(records = n()) %>% arrange(desc(records)) 

more_then_one_foreclosure <- foreclosures_work %>% group_by(FULLADDRESS) %>% summarise(number_of_foreclosures = n()) %>% count(number_of_foreclosures >= 2)
