...
Investigate data 
```{r}
unique(foreclosures_work$LIENTYPE)
...
more_then_one_foreclosure_count <- foreclosures_work %>% group_by(FULLADDRESS) %>% summarise(number_of_foreclosures = n()) %>% count(number_of_foreclosures >= 2)
...
highbid <- foreclosures_work %>% group_by()
```
